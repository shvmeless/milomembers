...
import css from './MemberEditorForm.module.scss'
...
import MemberDeleteButton from '../MemberDeleteButton/MemberDeleteButton'
import { NavigationContext } from '../../../contexts/navigation/navigationContext'

// COMPONENT
export default function MemberEditorForm (props: {
  member: Member
  className?: string
}): JSX.Element {
  ...
  // CONTEXT
  const { setLastCreatedMember, setSelectedMember } = useContext(CommonContext)
  const { updateMember } = useContext(MemberContext)
  const { setPath } = useContext(NavigationContext)

  // STATE
  const { form, updateForm } = useForm(member)
  ...
  // RETURN
  return <form className={classes(css.MemberEditorForm, className)} onSubmit={onSubmit}>
    ...
    <Submit className={css.submit} text='Modificar'/>
